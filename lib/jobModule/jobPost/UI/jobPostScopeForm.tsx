import { Form, Radio, Row, Col, Select, Input, Button } from "antd";
...
const JobPostScopeForm: FC<ScopeFormProps> = ({
        ...
        <Row>
          <Col span={24}>
            <div className={s.h_postJob_input_field}>
              ...
              >
                <Select placeholder={t("formItem.select")} onChange={onHandleSelectProjectScope}>
          ...
          <Col span={24}>
            <div className={ss.h_postJob_input_field} style={{ marginTop: "0px" }}>
              <Form.Item
          ...
          <Col span={12}>
            <div className={s.h_postJob_input_field}>
              ...
              >
                <Select placeholder={t("formItem.select")} onChange={onHandleChangeForSelectFieldValueAndFormData}>
              ...
              >
                <Select placeholder={t("formItem.select")} onChange={onHandleChangeForSelectFieldValueAndFormData}>
                  {successScoreList?.length > 0
                    ? successScoreList?.map((item: any) => <Select.Option key={item.value}>{item.title}</Select.Option>)
                    : null}
                </Select>
              </Form.Item>
            </div>
      ...
      <div className="collapse-card-padding" style={{ paddingBottom: "0px" }}>
        <Col span={24}>
          <div className={s.h_postJob_input_field}>
            ...
            >
              <Select placeholder={t("formItem.select")} onChange={onChangeDefaultLanguageEnglish}>
        ...
        ? language.map((item: any) =>
            item.language !== "" ? (
              <div className={s.h_jobPostForm_Scope_styled} key={language.indexOf(item)}>
                ...
                <InlineSVG src={closeIcon} height="auto" onClick={() => removeLanguageFromBothObject(item?.language)} />
              </div>
            ) : null
          )
        : null}
          ...
          <div className={ss.h_jobPostForm_Scope_language_styled}>
            <div className={ss.h_postJob_input_field} style={{ marginTop: "20px" }}>
              <Form.Item
                ...
                rules={[{ required: false, message: t("validationErrorMsgs.requireJobType") }]}
              >
                <Select placeholder={t("formItem.select")} onChange={onLanguageChangeHandler}>
          ...
          <Col span={24}>
            <div className={s.h_postJob_input_field}>
              ...
              >
                <Row>
                  <Col span={12}>
                    <div className={s.h_postJob_input_field} style={{ padding: "0px" }}>
                      <Form.Item
                        name="freelancer"
                        className={s.h_postJob_ant_form_item}
                        rules={[{ required: false, message: t("validationErrorMsgs.requireJobType") }]}
              ...
              >
                <Select placeholder={t("formItem.select")} onChange={onHandleChangeForSelectFieldValueAndFormData}>
                  {hoursBilledList?.length > 0
                    ? hoursBilledList?.map((item: any) => <Select.Option key={item.value}>{item.title}</Select.Option>)
                    : null}{" "}
                </Select>
              </Form.Item>
            </div>
