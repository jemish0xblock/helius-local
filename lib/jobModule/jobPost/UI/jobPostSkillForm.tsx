...
import { Form, Button, Collapse, Select } from "antd";
...
const JobPostSkillForm: FC<SkillFormProps> = ({
  form,
  ...
  const removeSkillsFromArrayList = (item: any) => {
    setSkillValues(skillValues.filter((value) => value.id !== item.id));
  };
      ...
      <div className="collapse-card-padding">
        ...
        {allSkillList?.map((item: any) =>
          item.title.split(" ")[1] === "Skills" ? (
            <div className={s.h_flex_wrap} key={`${item.id}${Math.random()}`} style={{ marginBottom: "50px" }}>
              {item?.items?.slice(0, 3).map((opt: any) => (
                <div className={s.h_postJob_button_layout_gray} key={opt.id}>
                  <Button
