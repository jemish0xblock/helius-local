...
import JobPostFormView from "./UI/jobPostForm";
import JobPostTypeSelection from "./UI/JobPostTypeSelection";

// const PostJobView = dynamic(() => import("./postJobView"));

interface JobPostControllerProps {
  jobPostType: string;
  jobIdFromUrl: string | any;
}
const PostJobController1: FC<JobPostControllerProps> = (props) => {
  ...
  const [formIsLoading, setFormIsLoading] = useState(isLoading);
    ...
    if (jobIdFromUrl) {
      ...
      if (formName === "JobPostFormDescription") {
        form.setFieldsValue({
          ...
          specialtyMain: jobPostReuseDetails?.speciality?.title,
  ...
  const onJobCategoryChange = (value: any) => {
    ...
    form.setFieldsValue({ speciality: null, specialtyMain: null, subCategory: null });
  ...
  const onJobSubCategoryChange = (value: any) => {
    ...
    form.setFieldsValue({ speciality: null, specialtyMain: null });
  ...
  const onJobSpecialityChange = (value: any) => {
    ...
    form.setFieldsValue({ specialtyMain: subSpecialityData.title });
  ...
  const formOnChangeMethod = (_item: any) => {
    // console.log("item: formOnChangeMethod", item);
  };
...
export default PostJobController1;
