import { LeftOutlined, InboxOutlined, LoadingOutlined } from "@ant-design/icons";
import { UploadProps, Layout, Row, Input, Col, Form, Button, message, Upload, DatePicker, Spin, Radio } from "antd";
...
import s from "./acceptSubmitProposal.module.less";
...
const AcceptSubmitProposalComponents: React.FC<CustomProps> = ({
  jobPostDetail,
  firstInputValue,
  handleOnFinish,
  secondInputValue,
  ...
  const convertAmountIntoPercentageValue = (amount: number, percentageValue: number) => {
    ...
    if (amount <= 500) {
      const result = (amount * percentageValue) / 100;
      diffAmount = amount - result;
    }
  ...
  useEffect(() => {
    ...
    if (sumAmount >= 0) {
      if (feeCharge >= 500) {
        result = (sumAmount * 20) / 100;
        diffAmount = sumAmount - result;
      }
      if (feeCharge > 500 && feeCharge <= 10000) {
        result = (sumAmount * 10) / 100;
        diffAmount = sumAmount - result;
      }
      if (feeCharge > 10000) {
        result = (sumAmount * 5) / 100;
        diffAmount = sumAmount - result;
      }
      if (jobPostDetail?.paymentType !== "hourly" && checkProjectType !== "project") {
        setFirstInputValue(sumAmount);
        ...
        <Row className={s.h_JobPostLists_wrapper_view} gutter={20}>
          <Col span={24} className={s.h_filter_wrapper_col}>
            <div className={s.submit_proposal_main_text}>
              <h1>Accept interview</h1>
              <p>
                Accept an interview for this job by replying and proposing terms. After you click Submit, this
                Invitation to Interview will become an "Active Interview". You will discuss the job and the client will
            ...
            >
              ...
              {/* {jobPostDetail?.paymentType === "hourly" ? (
                <div className={s.proposal_setting_text_section}>
                  <div className={`${s.proposal_setting_text_main_heading} ${s.h_proposal_terms_section}`}>
                    <h2>Terms</h2>
                    <p>Client’s budget: $15.00-$30.00 </p>
                  </div>
                  <SubmitProposalCommonComponent
                    data={jobPostDetail}
                    secondInputValue={secondInputValue}
...
export default AcceptSubmitProposalComponents;
